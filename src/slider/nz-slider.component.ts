...
@Component({
  ...
  providers: [{provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => NzSliderComponent), multi: true}],
  template: `
...
export class NzSliderComponent implements ControlValueAccessor,
  ...
  get nzRange(): boolean { return this._range; }
  ...
  get nzDots(): boolean { return this._dots; }
  ...
  onValueChange: (value: SliderValue) => void;  // Used by ngModel. BUG: onValueChange() will not success to effect the
                                                // "value" variable ( [(ngModel)]="value" ) when the first initializing,
                                                // except using "nextTick" functionality (MAY angular2's problem ?)
  ...
  setValue(val: SliderValue, isWriteValue: boolean = false): void {
    if (isWriteValue) {  // [ngModel-writeValue]: Formatting before setting value, always update current value, but
                         // trigger onValueChange ONLY when the "formatted value" not equals "input value"
      ...
      //   if (this.onValueChange) { // NOTE: onValueChange will be unavailable when writeValue() called at the first
      //   time
  ...
  getValueToOffset(value?: SliderValue): SliderValue {
    ...
    // TODO: using type guard, remove type cast
    return this.nzRange ? (normalizedValue as number[]).map(val => this.valueToOffset(val)) :
                          this.valueToOffset(normalizedValue as number);
  }

  writeValue(val: SliderValue): void {  // NOTE: writeValue will be called twice when initialized (may BUG? see:
                                        // https://github.com/angular/angular/issues/14988), here we just ignore the
                                        // first inited(the first the onValueChange will not registered)
  ...
  registerOnChange(fn: (value: SliderValue) => void): void { this.onValueChange = fn; }
  ...
  ngOnDestroy(): void { this.unsubscribeDrag(); }
  ...
  createDrag(): void {
    ...
    const mouse = {
      ...
      pluckKey: string[], filter(e: Event): boolean,
    ...
    [mouse, touch].forEach(source => {
      ...
      source.moveResolved$ = fromEvent(document, move)
                                 .pipe(
                                     filter(filterFunc), tap(this.utils.pauseEvent), pluck(...pluckKey),
                                     distinctUntilChanged(), map((position: number) => this.findClosestValue(position)),
  ...
  findClosestValue(position: number): number {
    ...
    // return the fixed
    return this.nzStep === null ? closest : parseFloat(closest.toFixed(this.utils.getPrecision(this.nzStep)));
  }

  valueToOffset(value: number): number { return this.utils.valueToOffset(this.nzMin, this.nzMax, value); }
  ...
  formatValue(value: SliderValue): SliderValue {  // NOTE: will return new value
    ...
    if (!this.checkValidValue(value)) {  // if empty, use default value
      ...
      // TODO: using type guard, remove type cast
      res = this.nzRange ? (value as number[]).map(val => this.utils.correctNumLimit(val, this.nzMin, this.nzMax)) :
  ...
  checkValidValue(value: SliderValue): boolean {
    ...
    if (isArray !== !!range) {  // value type not match
      throw new Error(
          `The "nzRange" can't match the "nzValue"'s type, please check these properties: "nzRange", "nzValue", "nzDefaultValue".`);
    }
    return true;
  }
  ...
  private _showHandleTooltip(handleIndex: number = 0): void {
    this.handles.forEach((handle, index) => { this.handles[index].active = index === handleIndex; });
  }

  private _hideAllHandleTooltip(): void { this.handles.forEach(handle => handle.active = false); }
