...
export class NzSliderMarksComponent implements OnChanges {
  ...
  @Input() set nzVertical(value: boolean) {  // Required
  ...
  trackById(
      index: number,
      attr: {id: number, value: number, offset: number, classes: {[key: string]: boolean}, style: object, label: Mark}):
      number {
    return attr.id;
  }
  ...
  buildAttrs(): void {
    ...
    this.attrs = this.nzMarksArray.map(mark => {
      ...
      let style: object;
      if (this.nzVertical) {
        style = {marginBottom: '-50%', bottom: `${(value - this.nzMin) / range * 100}%`};
      } else {
      ...
      return {id: value, value, offset, classes: {[`${this.nzClassName}-text`]: true}, style, label};
    });  // END - map
  }
...
export type Mark = string | {style: object; label: string;};

export class Marks { number: Mark; }
...
export class MarksArray extends Array<{value: number, offset: number, config: Mark}> {
  [index: number]: {value: number; offset: number; config: Mark;}
}
